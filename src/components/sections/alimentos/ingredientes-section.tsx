    ...
    >
      <div className="max-w-7xl mx-auto space-y-12 sm:space-y-16">
        ...
        <div className="max-w-4xl space-y-4 text-left">
          <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-[#183c6b]/10 text-[#183c6b] text-xs font-black uppercase tracking-widest">
          ...
          <h2 className="text-3xl sm:text-4xl lg:text-5xl font-black text-[#183c6b] tracking-tight leading-tight">
            ...
            <span className="block text-[#009539]">
        ...
        <div className="flex flex-wrap gap-2 sm:gap-3 p-1.5 rounded-2xl bg-slate-100/80 border border-slate-200/70 w-fit">
          {CATEGORIES.map((cat) => {
            ...
            return (
              ...
              >
                <Icon className={`w-4 h-4 ${isActive ? "text-[#02aeaa]" : "text-slate-400"}`} />
                <span>{cat.label}</span>
              </button>
            );
          })}
            ...
            return (
              ...
              >
                ...
                <div className="space-y-5">
                  ...
                  <div className="flex items-start justify-between gap-4">
                    <div className="space-y-1">
                      <span className="text-[10px] font-black uppercase tracking-wider text-[#02aeaa] block">
                        {item.categoryLabel}
                      </span>
                  ...
                  <div className="bg-white rounded-2xl p-5 border border-slate-200/80 shadow-2xs relative flex flex-col sm:flex-row items-center justify-between gap-5 h-56 sm:h-60 overflow-hidden group/stage">
                    ...
                    <div className="flex items-center justify-center p-3.5 bg-white rounded-2xl border border-slate-200/90 h-16 sm:h-20 w-40 sm:w-48 relative overflow-hidden shadow-xs shrink-0 self-start sm:self-center">
                      {item.logoUrl ? (
                        ...
                        <span className="text-sm font-black text-[#183c6b] tracking-tight truncate">
                          {item.name}
                        </span>
                      )}
                    </div>
                      ...
                      <div className="space-y-1.5">
                        ...
                        {item.benefits?.slice(0, 1).map((ben) => (
                          <div key={ben} className="flex items-start gap-2 text-xs text-slate-700 font-medium">
                            <CheckCircle2 className="w-3.5 h-3.5 text-[#02aeaa] shrink-0 mt-0.5" />
                  ...
                  {item.variants && item.variants.length > 0 && (
                    <div className="pt-1">
                      <button
                        ...
                        className="w-full flex items-center justify-between px-3.5 py-2.5 rounded-xl bg-slate-50 border border-slate-200/80 text-xs font-bold text-[#183c6b] hover:bg-slate-100/80 transition-colors cursor-pointer"
                      >
                        <span>Ver Variedades & Premezclas ({item.variants.length})</span>
                        <ChevronDown
                          className={`w-4 h-4 text-[#02aeaa] transition-transform duration-200 ${
                        ...
                        <div className="mt-2.5 space-y-3 p-3.5 rounded-xl bg-slate-50/90 border border-slate-200/70 animate-fade-in text-xs">
                          {item.variants.map((v) => (
                            <div key={v.name} className="space-y-1 pb-2 border-b border-slate-200/60 last:border-0 last:pb-0">
                              <h4 className="font-bold text-[#183c6b] text-xs">
                                {v.name}
                              </h4>
                ...
                <div className="mt-auto pt-4 border-t border-slate-100 flex items-center justify-between gap-3 text-xs text-slate-500">
                  ...
                  <span className="text-xs font-heading font-semibold text-slate-400">Grupo Mimesa</span>
                </div>
              </div>
            );
          })}
