  ...
  return (
    <div className="pt-2 flex flex-wrap gap-4">
      <button
        ...
        className="inline-flex items-center gap-2 px-8 py-4 rounded-full text-white text-xs sm:text-sm font-bold shadow-lg hover:shadow-xl hover:-translate-y-0.5 transition-all duration-300 cursor-pointer"
      ...
      <button
        ...
        className="inline-flex items-center gap-2 px-7 py-4 rounded-full text-slate-700 bg-white border border-slate-200 hover:border-slate-300 text-xs sm:text-sm font-bold shadow-xs hover:bg-slate-50 transition-all cursor-pointer"
      >
...
export function BrandProductsClient({ brand }: BrandProductsClientProps) {
  ...
  return (
    <div className="space-y-10">
        ...
        <div className="space-y-4">
          ...
          {brand.id === "ronco" && selectedLineId === "especialidades" && (
            <div className="flex items-center gap-2 pt-1 animate-fade-in">
              <span className="text-xs font-bold text-slate-400 pr-2">Sublínea:</span>
              <button
                type="button"
      ...
      {/* Grid de Packshots de Producto con min-height estable para prevenir colapso de scroll */}
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6 sm:gap-8 min-h-[500px]">
        {displayedProducts.map((product) => (
          <div
            key={product.id}
            className="group rounded-[2rem] bg-white border border-slate-200/90 p-6 sm:p-7 shadow-sm hover:shadow-xl transition-all duration-300 flex flex-col justify-between space-y-5 relative overflow-hidden"
          >
            {/* Top Badge */}
            <div className="flex items-center justify-between gap-2">
              <span
                className="text-[10px] font-extrabold uppercase tracking-wider px-2.5 py-1 rounded-full bg-slate-100 text-slate-700"
              >
